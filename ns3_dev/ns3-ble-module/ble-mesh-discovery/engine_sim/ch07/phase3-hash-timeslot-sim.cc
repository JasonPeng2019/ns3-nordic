...
static bool
GetActiveSlotInfo (const ble_mesh_node_t *state, uint32_t nowMs, ActiveSlotInfo *info)
{
  if (!state || !info)
    {
      return false;
  ...
  const ble_slot_assignment_t *slot = nullptr;
  if (state->clusterhead_id != BLE_MESH_INVALID_NODE_ID && state->cluster_slot.assigned)
    {
      slot = &state->cluster_slot;
      info->isClusterSlot = true;
    }
  else if (state->self_slot.assigned)
    {
      slot = &state->self_slot;
      info->isClusterSlot = false;
    }

  if (!slot || slot->tdma_slots == 0 || slot->frame_ms == 0)
    {
      return false;
    }

  uint32_t frameIdx = nowMs / slot->frame_ms;

  info->valid = true;
  info->channel = slot->channel_index;
  info->slot = slot->slot_index;
  info->frameIdx = frameIdx;
  info->iteration = static_cast<uint8_t> (frameIdx % 3);
  return true;
}
...
static const char TRACE_HEADER[] =
  ...
  "message_type,is_renouncement,class_id,direct_connections,score,hash,"
  "pdsf,last_pi,node_state,clusterhead_id,cluster_class,"
  "fdma_channel,tdma_slot,frame_idx,slot_event,slot_success,iteration";
...
public:
  ...
  void SetSimulateCollisions (bool enable);
...
private:
  ...
  bool m_simulateCollisions {true};
  mutable std::map<std::tuple<uint32_t, uint32_t, uint32_t, uint32_t>, uint32_t> m_collisionBuckets;
};
...
public:
  ...
  void SetTdmaFdma (uint32_t tdmaSlots, uint32_t fdmaChannels, Time frameDuration);
  void EnableCollisionModel (bool enable);
  void Start ();
  void ReceivePacket (Ptr<Packet> packet, int8_t rssi);
  const ble_mesh_node_t *GetNodeState () const;
  uint32_t GetNodeId () const;
  Vector GetPosition () const;
  void TraceSlotOutcome (const BleDiscoveryEngineWrapper::SlotOutcomeEvent &evt);
...
void
SimpleVirtualChannel::SetSimulateCollisions (bool enable)
{
  m_simulateCollisions = enable;
}
...
SimpleVirtualChannel::Transmit (uint32_t senderId, Ptr<Packet> packet) const
{
  ...
  ActiveSlotInfo slotInfo;
  bool hasSlot = GetActiveSlotInfo (m_nodes.at (senderId)->GetNodeState (),
  ...
  g_traceFile << nowMs << ",SEND," << senderId << ","
              ...
              << "" << ","  // clusterhead_id
              << "" << ","  // cluster_class
              << (hasSlot ? std::to_string (slotInfo.channel) : "") << ","
              << (hasSlot ? std::to_string (slotInfo.slot) : "") << ","
              << (hasSlot ? std::to_string (slotInfo.frameIdx) : "") << ","
              << "" << ","  // slot_event
              << "" << ","  // slot_success
              << (hasSlot ? std::to_string (slotInfo.iteration) : "")
              << "\n";
                               ...
                               int8_t rssi) const
{
  ...
  ActiveSlotInfo slotInfo;
  bool hasSlot = GetActiveSlotInfo (dstIt->second->GetNodeState (),
                                    static_cast<uint32_t> (nowMs),
                                    &slotInfo);
  bool collided = false;
  if (m_simulateCollisions && hasSlot)
    {
      auto key = std::make_tuple (receiverId, slotInfo.channel, slotInfo.slot, slotInfo.frameIdx);
      auto itCount = m_collisionBuckets.find (key);
      if (itCount == m_collisionBuckets.end ())
        {
          m_collisionBuckets[key] = 1;
        }
      else
        {
          itCount->second += 1;
  ...
  g_traceFile << nowMs << ",RECV,"
              ...
              << (hasSlot ? std::to_string (slotInfo.slot) : "") << ","
              << (hasSlot ? std::to_string (slotInfo.frameIdx) : "") << ","
              << (collided ? "COLLISION" : "") << "," // slot event
              << (collided ? 0 : 1) << "," // slot success
              << (hasSlot ? std::to_string (slotInfo.iteration) : "")
              << "\n";
...
EngineSimNode::EngineSimNode ()
  : m_engine (CreateObject<BleDiscoveryEngineWrapper> ()),
    ...
    m_fdmaChannels (1),
    m_tdmaSlots (8),
    m_frameDuration (MilliSeconds (100))
{
}
                          ...
                          Ptr<SimpleVirtualChannel> channel)
{
  ...
  m_engine->SetAttribute ("FdmaChannels", UintegerValue (m_fdmaChannels));
  m_engine->SetAttribute ("TdmaSlots", UintegerValue (m_tdmaSlots));
  m_engine->SetAttribute ("FrameDuration", TimeValue (m_frameDuration));
  m_engine->SetAttribute ("EnableDataPhase", BooleanValue (true));
  m_engine->TraceConnectWithoutContext ("SlotOutcome",
                                        MakeCallback (&EngineSimNode::TraceSlotOutcome, this));
...
void
EngineSimNode::SetTdmaFdma (uint32_t tdmaSlots, uint32_t fdmaChannels, Time frameDuration)
{
  m_tdmaSlots = tdmaSlots;
  m_fdmaChannels = fdmaChannels;
  m_frameDuration = frameDuration;
}

void
EngineSimNode::EnableCollisionModel (bool enable)
{
  m_engine->SetAttribute ("EnableCollisionModel", BooleanValue (enable));
}
...
void
EngineSimNode::TraceSlotOutcome (const BleDiscoveryEngineWrapper::SlotOutcomeEvent &evt)
{
  g_traceFile << Simulator::Now ().GetMilliSeconds () << ",SLOT," << m_nodeId << ","
              << "" << ","  // receiver
              << "" << ","  // originator
              << "" << ","  // ttl
              << "" << ","  // path length
              ...
              << "SLOT_EVENT" << ","
              << (evt.outcome == 0 ? 0 : 1) << ","
              << static_cast<uint32_t> (evt.iteration)
              << std::endl;
}
...
main (int argc, char *argv[])
{
  ...
  uint32_t fdmaChannels = 1;
  uint32_t tdmaSlots = 8;
  Time frameDuration = MilliSeconds (100);
  bool simulateCollisions = true;
  bool allowNoClusterheads = false;
  bool allowNoAlignedEdges = false;
  ...
  cmd.AddValue ("fdmaChannels", "Number of FDMA channels", fdmaChannels);
  cmd.AddValue ("tdmaSlots", "TDMA slots per frame", tdmaSlots);
  cmd.AddValue ("frameMs", "TDMA frame duration (ms)", frameDuration);
  cmd.AddValue ("simulateCollisions", "Enable slot collision model", simulateCollisions);
  cmd.AddValue ("allowNoClusterheads", "Allow sim to complete even if no clusterheads elected", allowNoClusterheads);
  cmd.AddValue ("allowNoAlignedEdges", "Allow sim to complete even if no edge aligns to a clusterhead", allowNoAlignedEdges);
  ...
  for (uint32_t i = 0; i < nodeCount; ++i)
    {
      ...
      node->SetTdmaFdma (tdmaSlots, fdmaChannels, frameDuration);
      node->EnableCollisionModel (simulateCollisions);
