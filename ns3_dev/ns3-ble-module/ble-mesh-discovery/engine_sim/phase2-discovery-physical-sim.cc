 ...
 *
 * This simulation adds realistic physical layer effects to the discovery engine:
 ...
 *
 * Unlike phase2-discovery-sim.cc which uses perfect virtual channels,
 * this simulation models real-world radio propagation effects.
 */
...
NS_LOG_COMPONENT_DEFINE ("Phase2PhysicalDiscoverySim");
...
main (int argc, char *argv[])
{
  ...
  double gridSpacing = 20.0;       // meters between nodes in grid
  ...
  std::string traceFile = "physical_simulation_trace.csv";
  ...
  cmd.AddValue ("spacing", "Grid spacing in meters", gridSpacing);
  ...
  LogComponentEnable ("Phase2PhysicalDiscoverySim", LOG_LEVEL_INFO);
  ...
  // Create nodes in a grid topology
  std::vector<Ptr<PhysicalSimNode>> nodes;
  nodes.reserve (nodeCount);

  uint32_t gridWidth = static_cast<uint32_t> (std::ceil (std::sqrt (nodeCount)));
  ...
  for (uint32_t i = 0; i < nodeCount; ++i)
    {
      ...
      // NS-3 Cartesian position
      Vector position (col * gridSpacing, row * gridSpacing, 0.0);

      // GPS coordinate (offset from base)
      double latOffset = (row * gridSpacing) / metersPerDegreeLat;
      double lonOffset = (col * gridSpacing) / metersPerDegreeLon;
